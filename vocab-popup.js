...
(function(){
  ...
  (function(){
    ...
    function createNextClone(){
      if (!origNext || !origPrev) return;
      if (nextClone) return;
      nextClone = origNext.cloneNode(true);
      nextClone.id = 'nextBtn_clone_mobile';
      nextClone.setAttribute('aria-hidden','false');
      // forward click to original
      nextClone.addEventListener('click', function(e){ e.preventDefault(); origNext.click(); });
      // insert after prev to appear on the same line as prev
      origPrev.parentNode.insertBefore(nextClone, origPrev.nextSibling);
      // hide original visually but keep for accessibility
      origNext.style.display = 'none';
      nextClone.style.display = 'inline-block';
    }

    function removeNextClone(){
      if (nextClone){
        try { nextClone.parentNode.removeChild(nextClone); } catch(e){}
        nextClone = null;
      }
    ...
    function applyIfNeeded(){
      ...
      if (!popupVisible){
        removeCounterCopy(); disconnectCounterObserver(); removeNextClone(); return;
      }
      if (mq.matches){
        createCounterCopy();
        observeCounter();

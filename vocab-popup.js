      ...
      <div id="vocabBody">
        ...
        <div class="card">
          <div class="card-lines">
            ...
            <div class="vocab-spacer"></div>
            <div class="vocab-line">
  <div class="muted">Kanji <span class="click-hint">(cliquer)</span></div>
  <div class="kanji-line">
    <button id="strokeBtn" class="stroke-btn" title="Ordre des traits">✎</button>
    <div id="jpTxt" class="big tag kanji-red">•••</div>
  </div>
  <div id="strokePanel" class="stroke-panel" aria-hidden="true">
    <div id="strokeTarget" tabindex="0" title="Cliquer pour le kanji suivant"></div>
    <div id="strokeInfo" class="stroke-meta">—</div>
  </div>
</div>
          </div>
...
(function(){
  ...
  // --- Kanji zoom on click: cycles through 3 sizes then resets ---
  try {
    const jp = $('jpTxt');
    if (jp) {
      jp.classList.add('clickable');
      let level = 0;
      jp.addEventListener('click', () => {
        level = (level + 1) % 4; // 0 -> 1 -> 2 -> 3 -> 0
        jp.classList.remove('kanji-zoom-1','kanji-zoom-2','kanji-zoom-3');
        if (level === 1) jp.classList.add('kanji-zoom-1');
        else if (level === 2) jp.classList.add('kanji-zoom-2');
        else if (level === 3) jp.classList.add('kanji-zoom-3');
      });
    }
  } catch(_) {}
...
function updateStrokeInfo(row){
          try{
            const n = (lastCharData && Array.isArray(lastCharData.strokes)) ? lastCharData.strokes.length : null;
            const kana = row && row.Kana ? String(row.Kana) : '';
            const romaji = row && row.Romaji ? String(row.Romaji) : '';
            const parts = [];
            if (Number.isInteger(n)) parts.push(n + ' traits');
            if (kana) parts.push(kana);
            if (romaji) parts.push(romaji);
            if (strokeInfo) strokeInfo.textContent = parts.length ? parts.join(' · ') : '—';
          } catch(_){ if (strokeInfo) strokeInfo.textContent = '—'; }
        }

function refreshKanjiSeq(row){
  CURRENT_ROW = row || null;
  KSEQ = extractKanji(row?.Japonais || '');
  kIdx = 0;
  if (strokeBtn) strokeBtn.disabled = KSEQ.length === 0;
  if (strokePanel) { strokePanel.classList.remove('show'); strokePanel.setAttribute('aria-hidden','true'); }
}

async function playStroke(){
  if (!KSEQ.length) return;
  const ch = KSEQ[kIdx];
  kIdx = (kIdx + 1) % KSEQ.length;
  const HW = await ensureHW();
  if (!writer){
    writer = HW.create(strokeTargetId, ch, {
      width: 160, height: 160, padding: 6,
      showCharacter: true, showOutline: true,
      drawingColor: '#c62828',
      charDataLoader: function(char, onComplete){
        const url = 'https://cdn.jsdelivr.net/gh/mnako/hanzi-writer-data-ja@master/data/' + encodeURIComponent(char) + '.json';
        fetch(url).then(r=>r.json()).then(d=>{ lastCharData = d; onComplete(d); try{ updateStrokeInfo(CURRENT_ROW); }catch(_){} }).catch(()=>onComplete(null));
      }
    });
  } else {
    writer.setCharacter(ch);
        try{ updateStrokeInfo(CURRENT_ROW); }catch(_){}
  }
  if (strokePanel && !strokePanel.classList.contains('show')){
    strokePanel.classList.add('show');
        try{ updateStrokeInfo(CURRENT_ROW); }catch(_){}
    strokePanel.setAttribute('aria-hidden','false');
  }
  try { writer.animateCharacter(); } catch(_){}
}
if (strokeBtn) strokeBtn.addEventListener('click', playStroke);
if (strokeTarget) strokeTarget.addEventListener('click', playStroke);
  ...
  function targetText(row){
    const qmode = (questionMode ? questionMode.value : 'fr2ja') || 'fr2ja';
    const mode  = (inputMode ? inputMode.value : 'kana') || 'kana';
  ...
  // ---------- UI ----------
  function setModeUI(){
  if (!inputMode || !questionMode) return;
  const qmode = (questionMode ? questionMode.value : 'fr2ja');
  const disable = (qmode === 'fr2ja');
  inputMode.disabled = disable;
  inputMode.title = disable ? 'Sans objet en FR → JA' : 'Mode de saisie';
}
  ...
  if (!currentList.length){
    try { if (strokeBtn){ strokeBtn.disabled = true; } if (strokePanel){ strokePanel.classList.remove('show'); strokePanel.setAttribute('aria-hidden','true'); } } catch(_) {}
  ...
  refreshKanjiSeq(row); try{ updateStrokeInfo(row); }catch(_){}
  ...
  function next(){ if (currentList.length){ index = (index + 1) % currentList.length; render(); } if (strokePanel){ strokePanel.classList.remove('show'); strokePanel.setAttribute('aria-hidden','true'); } }
  function prev(){ if (currentList.length){ index = (index - 1 + currentList.length) % currentList.length; render(); } if (strokePanel){ strokePanel.classList.remove('show'); strokePanel.setAttribute('aria-hidden','true'); } }
